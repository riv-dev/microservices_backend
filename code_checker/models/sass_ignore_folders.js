...
URLsToCheck.initialize_db = function(env, call_back) {
  ...
  this.db.query('CREATE TABLE IF NOT EXISTS sass_ignore_folders (id int NOT NULL AUTO_INCREMENT, project_id int NOT NULL, relative_folder_path varchar(32), UNIQUE (project_id, relative_folder_path), PRIMARY KEY(id), FOREIGN KEY (project_id) REFERENCES code_checker_projects(project_id) ON DELETE CASCADE);', function(err) {
    if(err) {
      console.log(err);
    } 
  });
...
URLsToCheck.find_all = function (query, call_back) {
  ...
  this.db.query('SELECT * FROM sass_ignore_folders;', function(err, results, fields) {
...
URLsToCheck.find_all_by_project_id = function (project_id, call_back) {
  ...
  this.db.query("SELECT * FROM sass_ignore_folders WHERE project_id = ?;", [project_id], function (err, results, fields) {
    call_back(err, results, fields);
  });
}
...
URLsToCheck.add = function(body, call_back) {
  ...
  this.db.query("INSERT into sass_ignore_folders (" + addStringArray.join(", ") +") values ("+ addMarksArray.join(",")+");", addValuesArray, function(err, results, fields) {
    if(err) {
      console.log(err);
    }
    call_back(err, results, fields);
...
URLsToCheck.update = function(id, body, call_back) {
  ...
  this.db.query("UPDATE sass_ignore_folders SET " + updateStringArray.join(", ") + " WHERE project_id = ?;", updateValuesArray, function(err, results, fields) {
    call_back(err, results, fields);
  });  
}
...
URLsToCheck.delete = function(id, call_back) {
  ...
  this.db.query("DELETE from sass_ignore_folders WHERE id = ?;", [id], function(err, results, fields) {
    call_back(err, results, fields);
  });    
}
...
URLsToCheck.delete_all = function(project_id, call_back) {
  ...
  this.db.query("DELETE from sass_ignore_folders WHERE project_id = ?;", [project_id], function(err, results, fields) {
    call_back(err, results, fields);
  });
}
