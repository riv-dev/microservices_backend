...
Patches.apply_patch = function(env, call_back) {
  ...
  this.db.query('ALTER TABLE code_checker_projects ADD COLUMN source_username varchar(255);', function(err) {
    if(err) {
      console.log(err);
    } 
  });  

  this.db.query('ALTER TABLE code_checker_projects ADD COLUMN source_password varchar(255);', function(err) {
    if(err) {
      console.log(err);
    } 
  });  
