...
Patches.apply_patch = function(env, call_back) {
  ...
  this.db.query('ALTER TABLE code_checker_projects ADD COLUMN check_sass BOOLEAN DEFAULT true;', function(err) {
    if(err) {
      console.log(err);
    } 
  });  

  this.db.query('ALTER TABLE code_checker_projects ADD COLUMN check_html BOOLEAN DEFAULT true;', function(err) {
    if(err) {
      console.log(err);
    } 
  });  
