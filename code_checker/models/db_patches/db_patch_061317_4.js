...
Patches.apply_patch = function(call_back) {
  ...
  this.db.query('ALTER TABLE project_users CHANGE status_code status varchar(255) DEFAULT "active";', function(err) {
    if(err) {
      console.log(err);
    } 
  });

  this.db.query('UPDATE project_users SET status = "active";', function(err) {
    if(err) {
      console.log(err);
    } 
  });
