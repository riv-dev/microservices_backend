...
Patches.apply_patch = function(env, call_back) {
  ...
  this.db.query('ALTER TABLE result_messages CHANGE msg_type validator varchar(32);', function(err) {
    if(err) {
      console.log(err);
    } 
  });

  this.db.query('ALTER TABLE result_messages CHANGE msg_level level varchar(32);', function(err) {
    if(err) {
      console.log(err);
    } 
  });

  this.db.query('ALTER TABLE result_messages CHANGE msg message varchar(255);', function(err) {
    if(err) {
      console.log(err);
    } 
  });
