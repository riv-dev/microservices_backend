...
Projects.find_all = function (query, call_back) {
  ...
  //Project Ranking
  var orderByClause = " ORDER BY pinned DESC, LENGTH(status) ASC, CASE WHEN status!='finished' THEN -deadline WHEN status='finished' THEN deadline END DESC, -value ASC, -start_date DESC, id DESC";
...
Projects.find_all_by_user_id = function(query, user_id, call_back) {
  ...
  //Project Ranking
  var orderByClause = " ORDER BY project_users.user_pinned DESC, LENGTH(projects.status) ASC, CASE WHEN projects.status!='finished' THEN -projects.deadline WHEN projects.status='finished' THEN projects.deadline END DESC, -projects.value ASC, -projects.start_date DESC, projects.id DESC";
