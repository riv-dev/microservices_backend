...
Patches.apply_patch = function(env, call_back) {
  ...
  this.db.query('UPDATE projects SET status = "dump" WHERE status = "new";', function(err) {
    if(err) {
      console.log(err);
    } 
  });

  this.db.query('ALTER TABLE projects ALTER status SET DEFAULT "dump";', function(err) {
    if(err) {
      console.log(err);
    } 
  });
