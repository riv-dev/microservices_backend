...
Patches.apply_patch = function(call_back) {
  ...
  this.db.query('ALTER TABLE tasks ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;', function(err) {
    if(err) {
      console.log(err);
    } 
  });

  this.db.query('ALTER TABLE tasks ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP;', function(err) {
    if(err) {
      console.log(err);
    } 
  });  
